...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the XAP and zXAP money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 53241, 12228));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in XAP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in XAP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, apollon, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in XAP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-xapstake=<n>", strprintf(_("Enable or disable staking functionality for XAP inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zxapstake=<n>", strprintf(_("Enable or disable staking functionality for zXAP inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:53241"));
...
LogPrintf("Cannot downgrade wallet nMaxVersion: %d, nVersion: %d", nMaxVersion, pwalletMain->GetVersion());
        ...
        zwalletMain = new CzXAPWallet(pwalletMain->strWalletFile);
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    ...
    LogPrintf("Anonymize Apollon Amount %d\n", nAnonymizeXapxAmount);
    ...
    /* Denominations
       ...
       For example:
       1XAP+1000 == (.1XAP+100)*10
       10XAP+10000 == (1XAP+1000)*10
    */
